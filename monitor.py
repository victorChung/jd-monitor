...
def fetch_current_price(sku_id):
    """获取当前价格（单位：元）"""
...
def fetch_product_name(sku_id):
    """尝试从商品页面获取名称（备用，优先使用 config 中的 name）"""
...
def generate_html(items_info, output_file):
    """生成美观的 HTML 报告"""
    html_template = """
    <!DOCTYPE html>
    <html lang="zh-CN">
    ...
    html_content = html_template.format(
        ...
        rows=rows_html,
        threshold_ratio=items_info[0]["threshold_ratio"] if items_info else "1.1"
    )
...
def main():
    ...
    for idx, item_cfg in enumerate(items_config):
        ...
        # 获取当前价格
        current_price = fetch_current_price(sku_id)
    ...
    # 生成 HTML
    generate_html(items_info, html_output)
    print("监控完成！")
